...
const COMPACT_CELL_STYLES: Record<'success' | 'mixed' | 'failed' | 'none', { bg: string; border: string }> = {
  success: { bg: '#BBF7D0', border: '#4ADE80' },
  mixed: { bg: '#FED7AA', border: '#FB923C' },
  failed: { bg: '#FECACA', border: '#F87171' },
...
function CompactHeatmap({ values }: { values: HeatmapValue[] }) {
  ...
  return (
    <div>
      ...
      <div className="grid grid-rows-7 grid-flow-col" style={{ gap: '0.3mm', width: 'fit-content' }}>
        {weeks.flatMap((week) =>
          week.map((day) => {
            if (!day.inRange) {
              return <span key={day.date} className="block rounded-[0.4mm] border border-transparent"
                style={{ width: '3mm', height: '3mm' }} />;
            }
            const kind = getCompactCellKind(day.value);
            const cs = COMPACT_CELL_STYLES[kind];
            return <span key={day.date} title={getTooltipText(day.date, day.value)}
              className="block rounded-[0.4mm] border"
              style={{ width: '3mm', height: '3mm', backgroundColor: cs.bg, borderColor: cs.border }} />;
          })
        )}
      </div>
